...
const Dashboard = () => {
    ...
    const fetchGambar = async () => {
        try {
            const response = await axios.get('http://127.0.0.1:8000/api/picture?id_user=' + Id);
    ...
    const fetchBarang = async () => {
        try {
            const response = await axios.get('http://127.0.0.1:8000/api/barang?id_user=' + Id);
            setBarang(response.data.length); // Sesuaikan dengan struktur respons yang sesuai
        } catch (error) {
                        ...
                        <div className="card-body">
                            <div className="row no-gutters align-items-center">
                                <div className="col mr-2">
                                    <div className="text-xs font-weight-bold text-success text-uppercase mb-1">
                                        Jumlah Barang</div>
                                    <div className="h5 mb-0 font-weight-bold text-gray-800">{barang}</div>
                                </div>
                                <div className="col-auto">
                                    <i className="bi bi-image fs-3 text-gray-300"></i>
                                </div>
